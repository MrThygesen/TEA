...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (telegramChatId && eventId) {
        await pool.query(
          `UPDATE registrations SET has_paid = TRUE WHERE event_id = $1 AND telegram_user_id = $2`,
        ...
        await fetch(`${process.env.RENDER_API_BASE}/notify-payment`, {
          ...
          body: JSON.stringify({ telegram_chat_id: telegramChatId, event_id: eventId }),
        });
