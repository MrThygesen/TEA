...
export default function WebAccessSBT () {
  ...
  useEffect(() => {
    async function fetchSBTTypes() {
      try {
        const sbtTypeIds = await readContract({
          address: CONTRACT,
          abi: WebAccessSBTV5_ABI.abi,
          functionName: 'sbtTypeIds',
          chainId,
        })

        const entries = await Promise.all(
          sbtTypeIds.map(async (id) => {
            const sbt = await readContract({
              address: CONTRACT,
              abi: WebAccessSBTV5_ABI.abi,
        ...
        setOptions(entries)
        if (entries.length > 0) setTypeId(entries[0].id)
      } catch (err) {
        console.error('Failed to fetch SBT types:', err)
      }
    }
    if (chainId) fetchSBTTypes()
  }, [chainId])
  ...
  useEffect(() => {
    async function fetchTokens () {
      if (!isConnected || !chainId) return setTokens([])
      try {
    ...
    fetchTokens()
  }, [address, isConnected, hash, chainId])
  ...
  async function handleClaim () {
    try {
      ...
      if (simError)
        return toast.error(simError.shortMessage || simError.message || 'Simulation error')
      if (!sim?.request)
      ...
      <div className="bg-white p-8 rounded-3xl shadow-xl border border-gray-200 space-y-8">
        ...
        {tokens.length > 0 && (
          <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 mt-4">
            <h3 className="text-sm font-semibold text-gray-800 mb-2">🎫 Your Claimed Tokens</h3>
