// components/AdminSBTManager1new.js
'use client'
...
export default function AdminSBTManager() {
  ...
  const handleSetOrganizer = () => {
    writeContract({
      ...
      args: [typeId, organizer],
      onSuccess: () => toast.success('✅ Organizer set'),
      onError: (err) => toast.error(err.message),
    })
  }
  ...
  return (
    <div className="p-6 border rounded max-w-xl mx-auto bg-white text-black space-y-4">
      ...
      <div>
        ...
        >
          {predefinedTypes.map(({ id, label }) => (
            <option key={id.toString()} value={id.toString()}>{label}</option>
          ))}
        </select>
      </div>
      ...
      <div className="flex space-x-4">
        ...
        <button onClick={handleActivate} className="bg-green-600 text-white px-4 py-2 rounded">
          ✅ Activate Type
        </button>
      </div>
      ...
      <h3 className="text-lg font-bold">📋 Set Organizer</h3>
      <div>
        <input
          type="text"
