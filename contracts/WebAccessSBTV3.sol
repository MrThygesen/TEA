    ...
    UUPSUpgradeable
{
    ...
    /**
     * @notice Create a new SBT campaign.
     * @param typeId        Unique ID (must not exist yet).
     * @param uri           Metadata link (GitHub raw or IPFS).
     * @param burnable_     Whether holders can burn this token.
     * @param maxSupply_    Maximum # of tokens mintable.
     * @param useWhitelist  If true, only whitelisted wallets may claim.
     */
    ...
    /** Toggle active status (claimable) */
    ...
    /** Optional airdrop / invite by admin */
    function airdropTo(uint256 typeId, address[] calldata users) external onlyOwner {
        ...
        for (uint256 i = 0; i < users.length && t.minted < t.maxSupply; ++i) {
            ...
            _mintToken(user, typeId);       // re‑uses internal mint helper
